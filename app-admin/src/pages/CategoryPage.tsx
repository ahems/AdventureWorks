...
const CategoryPage: React.FC = () => {
  ...
  const catIdNum = categoryId ? parseInt(categoryId) : 0;

  const { data: category } = useAdminCategoryById(catIdNum);
  const { data: subcategories = [] } =
    useAdminSubcategoriesByCategory(catIdNum);

  // Fetch products: by specific subcategory or all subcategories in category
  const subcategoryIds = React.useMemo(
    () => subcategories.map((s) => s.ProductSubcategoryID),
    [subcategories],
  );
  const { data: allSubcatProducts = [], isLoading: allLoading } =
    useAdminProductsBySubcategoryIds(subcategoryIds);
  const { data: filteredSubcatProducts = [], isLoading: filteredLoading } =
    useAdminProductsBySubcategory(selectedSubcategory);
  // First page of reviews only (used to build per-product rating averages)
  ...
  // Build a productId -> averageRating map from cached reviews
  const ratingMap = React.useMemo(() => {
    ...
    for (const [id, ratings] of groups) {
      map.set(id, ratings.reduce((a, b) => a + b, 0) / ratings.length);
    }
    return map;
  }, [allReviews]);

  const isLoading = selectedSubcategory ? filteredLoading : allLoading;

  const baseProducts = React.useMemo(() => {
    if (selectedSubcategory) return filteredSubcatProducts;
    return allSubcatProducts;
  }, [selectedSubcategory, filteredSubcatProducts, allSubcatProducts]);
  ...
  const allProducts = React.useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "rating":
        return sorted.sort(
          (a, b) =>
            (ratingMap.get(b.ProductID) ?? 0) -
            (ratingMap.get(a.ProductID) ?? 0),
        );
      case "sku":
                ...
                <div className="doodle-card p-4 mt-6 flex items-center justify-center gap-2">
                  ...
                  {getPageNumbers().map((page, index) =>
                    page === "..." ? (
                      <span
                        key={`ellipsis-${index}`}
                        className="px-2 font-doodle text-doodle-text/50"
                      >
                        ...
                      </span>
                    ) : (
