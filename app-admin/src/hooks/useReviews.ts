...
// Utility function to get average rating for any product without hooks
export const getProductAverageRating = (productId: number): number => {
  const storedReviews = localStorage.getItem('adventureworks_reviews');
  const allReviews: { productId: number; rating: number }[] = storedReviews 
