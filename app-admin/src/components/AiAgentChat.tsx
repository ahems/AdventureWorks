...
import { Bot, Send, User, Loader2, Terminal, ChevronDown, ChevronUp, Sparkles, Trash2, Plus, MessageSquare, X, Search, Lightbulb } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { ScrollArea } from '@/components/ui/scroll-area';
import { AgentMessage, processAgentMessage, RPC_TOOLS } from '@/services/mockRpcServer';
...
const getInitialMessage = (): AgentMessage => ({
  role: 'assistant',
  content: `👋 Hello! I'm your **AdventureWorks AI Assistant**.\n\nI have access to all your business data through an RPC server. Try asking me:\n\n• "Show me the stats"\n• "Who are the top customers?"\n• "Show pending orders"\n• "Revenue by state"`,
});
...
const loadConversations = (): { conversations: Conversation[]; activeId: string } => {
  try {
...
const AiAgentChat: React.FC = () => {
  ...
  const activeConversation = data.conversations.find(c => c.id === data.activeId) || data.conversations[0];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updateActiveConversation(conv => {
      ...
      return {
        ...
        title: isFirstUserMessage ? userInput.slice(0, 30) + (userInput.length > 30 ? '...' : '') : conv.title,
      };
    });
    ...
    try {
      const response = await processAgentMessage(userInput, messages);
      updateActiveConversation(conv => ({
        ...conv,
  ...
  const suggestedPrompts = [
    { icon: '🔍', title: 'Product Success', prompt: 'Analyze product 749 success metrics and give me AI insights' },
    { icon: '📈', title: 'Sales Report', prompt: 'Generate a sales report with breakdown by status' },
    { icon: '👥', title: 'Customer Report', prompt: 'Generate a customer report sorted by spending' },
    { icon: '📦', title: 'Inventory Report', prompt: 'Generate an inventory report sorted by popularity' },
    { icon: '🛒', title: 'Create Order', prompt: 'Create a new order for customer 1001 with product 749' },
    { icon: '🔄', title: 'Update Order', prompt: 'Update order 70004 status to processing' },
    { icon: '✏️', title: 'Update Customer', prompt: 'Update customer 1001 email to new@example.com' },
    { icon: '🎨', title: 'Generate Image', prompt: 'Generate a product image for product 749 in 3D render style' },
  ];
  ...
  const handleSuggestedPrompt = async (prompt: string) => {
    ...
    updateActiveConversation(conv => {
      ...
      return {
        ...conv,
        messages: [...conv.messages, userMessage],
        title: isFirstUserMessage ? prompt.slice(0, 30) + (prompt.length > 30 ? '...' : '') : conv.title,
      };
    });
    ...
    try {
      const response = await processAgentMessage(prompt, messages);
      updateActiveConversation(conv => ({
        ...conv,
        messages: [...conv.messages, response],
      }));
  ...
  const formatContent = (content: string) => {
    return content.split('\n').map((line, i) => {
      ...
      if (line.startsWith('• ') || line.startsWith('  • ')) {
        return <div key={i} className="pl-2" dangerouslySetInnerHTML={{ __html: line }} />;
      }
    ...
    <div className="doodle-card flex flex-col h-[500px] relative">
        ...
        <div className="absolute inset-0 z-20 flex">
          <div className="w-72 bg-doodle-bg border-r-2 border-doodle-text/20 flex flex-col h-full">
            <div className="p-3 border-b-2 border-doodle-text/20 flex items-center justify-between">
              <span className="font-doodle font-bold text-sm text-doodle-text">Conversations</span>
              ...
              <div className="border-b-2 border-doodle-text/10 max-h-48 overflow-auto">
                {searchResults.length === 0 ? (
                  <div className="p-3 text-center">
                    <p className="font-doodle text-xs text-doodle-text/50">No results found</p>
                  </div>
                ) : (
          ...
          <div className="flex-1 bg-doodle-text/20" onClick={() => setShowThreads(false)} />
        </div>
      )}
      ...
      {showTools && (
        <div className="p-3 bg-doodle-text/5 border-b-2 border-doodle-text/20 max-h-40 overflow-auto">
          <p className="font-doodle text-xs font-bold text-doodle-text mb-2">Available RPC Methods:</p>
              ...
              <div className={`max-w-[85%] ${
                ...
                {msg.toolCalls && msg.toolCalls.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-doodle-text/10">
                    <p className="font-mono text-xs text-doodle-text/50 mb-1">RPC Calls:</p>
                    {msg.toolCalls.map((call, i) => (
                      <div key={i} className="font-mono text-xs text-doodle-green/80 flex items-center gap-1">
                        <Terminal className="w-3 h-3" />
                        {call.method}({Object.keys(call.params).length > 0 ? JSON.stringify(call.params) : ''})
                        <span className="text-doodle-text/40">
                          {call.result.executionTime.toFixed(0)}ms
                        </span>
                      </div>
                    ))}
                  </div>
          ...
          {showSuggestedPrompts && !isLoading && (
            <div className="mt-4 pt-4 border-t-2 border-doodle-text/10">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-doodle text-sm font-bold text-doodle-text/70">Try asking:</span>
              </div>
            ...
            <div className="flex gap-3">
              ...
              <div className="bg-doodle-bg border-2 border-doodle-text/20 p-3 rounded-lg">
                <div className="flex items-center gap-2 text-doodle-text/60">
                  ...
                  <span className="font-doodle text-sm">Querying RPC server...</span>
                </div>
              </div>
            </div>
          )}
      ...
      {/* Input */}
      <form onSubmit={handleSubmit} className="p-4 border-t-2 border-doodle-text/20">
