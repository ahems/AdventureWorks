...
-- Step 5b: Expand Document.Title column to accommodate original CSV data
-- Original: nvarchar(50) - Too small for some document titles in the community CSV
-- Updated: nvarchar(200) - Accommodates all original Document.csv values without truncation

IF EXISTS (SELECT 1 FROM sys.tables WHERE object_id = OBJECT_ID(N'[Production].[Document]'))
BEGIN
    ALTER TABLE [Production].[Document]
    ALTER COLUMN [Title] [nvarchar](200) NOT NULL;
    PRINT 'Document.Title column updated to nvarchar(200)';
END;
