...
const NotifyWhenAvailable: React.FC<NotifyWhenAvailableProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !email.includes("@")) {
      toast({
        title: t("notifyWhenAvailable.invalidEmail"),
        description: t("notifyWhenAvailable.pleaseEnterValidEmail"),
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      title: t("notifyWhenAvailable.youreOnTheList"),
      description: t("notifyWhenAvailable.emailWhenInStock"),
    });
      ...
      <DialogContent className="bg-white border-2 border-doodle-text max-w-md">
        <DialogHeader>
          <DialogTitle className="font-doodle text-xl text-doodle-text flex items-center gap-2">
            ...
            {t("notifyWhenAvailable.getNotified")}
          </DialogTitle>
          <DialogDescription className="font-doodle text-doodle-text/70">
            {t("notifyWhenAvailable.emailWhenBackInStock")}
          </DialogDescription>
        </DialogHeader>
        ...
        {isSuccess ? (
          <div className="py-8 text-center">
            ...
            <p className="font-doodle text-lg font-bold text-doodle-text">
              {t("notifyWhenAvailable.youreAllSet")}
            </p>
            <p className="font-doodle text-sm text-doodle-text/70 mt-1">
              {t("notifyWhenAvailable.notifyWhenAvailable")}
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label
                ...
                className="font-doodle text-sm font-bold text-doodle-text"
              >
                {t("notifyWhenAvailable.emailAddress")}
              </label>
              <Input
                id="notify-email"
                type="email"
                placeholder={t("notifyWhenAvailable.emailPlaceholder")}
                value={email}
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  {t("notifyWhenAvailable.submitting")}
                </>
              ) : (
                <>
                  <Bell className="w-4 h-4" />
                  {t("notifyWhenAvailable.notifyMe")}
                </>
              )}
            </button>

            <p className="font-doodle text-xs text-doodle-text/50 text-center">
              {t("notifyWhenAvailable.onlyUseEmailForProduct")}
            </p>
          </form>
        )}
      </DialogContent>
