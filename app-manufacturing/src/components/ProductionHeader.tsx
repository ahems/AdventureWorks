...
const primaryNav = [
  { to: '/define', label: 'Define', icon: ClipboardList, stage: 1, desc: 'What we make — product catalog, models, categories and specs.' },
  { to: '/engineer', label: 'Engineer', icon: Cog, stage: 2, desc: 'How we make it — bills of materials, routings and manufacturing locations.' },
  { to: '/plan', label: 'Plan', icon: Calendar, stage: 3, desc: 'When we make it — demand, work orders, scheduling and planning intelligence.' },
  { to: '/execute', label: 'Execute', icon: Play, stage: 4, desc: 'Making it now — shop floor, work order tracking, workforce and finished-good tracker.' },
  { to: '/supply', label: 'Supply', icon: Truck, stage: 5, desc: 'Sourcing components — vendors, purchase orders and supply-chain risk.' },
  { to: '/receive', label: 'Receive', icon: PackageCheck, stage: 6, desc: 'Closing the loop — inventory by location and product, costing and margin analysis.' },
];
...
const secondaryNavGroups = [
  ...
  },
  {
    label: 'Reference',
    items: [
      { to: '/engineer/locations', label: 'Locations', icon: MapPin },
    ],
  },
  {
    label: 'Admin',
        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="font-doodle text-xs text-doodle-text/60 -mt-1">Production Hub</span>
            </div>
          </Link>
              ...
              <Tooltip key={item.to}>
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    <span className="text-doodle-accent/60 text-xs mr-0.5">{item.stage}.</span>
                    {item.label}
                  </Link>
                </TooltipTrigger>
                <TooltipContent side="bottom" className="max-w-[240px] font-doodle text-xs">
                  <p className="font-bold mb-0.5">{item.stage}. {item.label}</p>
                  <p>{item.desc}</p>
                </TooltipContent>
              </Tooltip>
            ))}
        ...
        {mobileMenuOpen && (
          <nav className="md:hidden py-4 border-t-2 border-doodle-text border-dashed">
            <div className="flex flex-col gap-3">
              <Link to="/" className="font-doodle text-lg text-doodle-text hover:text-doodle-accent py-2" onClick={() => setMobileMenuOpen(false)}>
                ✦ Dashboard
              </Link>
              {primaryNav.map((item) => (
                <Link
              ...
              {secondaryNavGroups.map((group) => (
                <div key={group.label} className="border-t-2 border-dashed border-doodle-text/30 my-2 pt-2">
                  ...
                  {group.items.map((item) => (
                    <Link key={item.to} to={item.to} className="font-doodle text-base text-doodle-text/70 hover:text-doodle-accent py-1 block" onClick={() => setMobileMenuOpen(false)}>
                      ✦ {item.label}
                    </Link>
                  ))}
                </div>
