  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Master controls for the underlying manufacturing and supply-chain
          simulators. These actions affect the demo environment, not real
          production data.
        </CardDescription>
      </CardHeader>
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Halts every active manufacturing operation on the shop floor
                immediately. New work orders won't start until the simulation is
                resumed (e.g. by scheduling a new production run).
              </p>
              <p className="text-xs text-muted-foreground mt-1 italic">
                Use to pause the demo cleanly before a reset, or to clear a
                runaway state.
              </p>
            </div>
            <Button
        ...
        <div className="border rounded-lg p-4 space-y-2">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Wipes vendor inventory and re-seeds every supplier's stock back
                to its starting quantities. In-flight purchase orders are
                cleared.
              </p>
              <p className="text-xs text-muted-foreground mt-1 italic">
                Use when the demo's vendor stock has been depleted or you want a
                clean slate for a presentation.
              </p>
            </div>
            <Button
              variant="destructive"
            ...
            >
              <RefreshCw
                className={`h-4 w-4 mr-1 ${resetMutation.isPending ? "animate-spin" : ""}`}
              />{" "}
              Reset
            </Button>
          </div>
        ...
        <div className="border rounded-lg p-4 space-y-2">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Bootstraps the supply-chain simulator from scratch — populates
                the vendor catalog, lead times, prices, and initial stock. Safe
                on a fresh environment; usually idempotent.
              </p>
              <p className="text-xs text-muted-foreground mt-1 italic">
                Use for first-time setup of a new demo backend, or after a
                deeper backend wipe where Reset alone isn't enough.
              </p>
            </div>
            <Button
              variant="outline"
            ...
            >
              <PlayCircle
                className={`h-4 w-4 mr-1 ${initMutation.isPending ? "animate-spin" : ""}`}
              />{" "}
              Initialize
            </Button>
          </div>
