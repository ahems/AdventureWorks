...
export function VendorRestockCard() {
  ...
  const { data: vendors, isLoading } = useQuery({ queryKey: ['supply-vendors'], queryFn: fetchVendors });
  ...
  const sorted = (vendors ?? []).slice().sort((a, b) => a.vendor.name.localeCompare(b.vendor.name));
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          <p>Tops up a single vendor's available stock without resetting the rest of the supply chain. Other vendors and order history are untouched.</p>
          <p className="mt-1 text-xs italic">Use when one supplier has run dry during a demo and you want to keep procurement flowing without a full reset.</p>
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="flex flex-col sm:flex-row gap-3 sm:items-end">
          ...
          >
            <RefreshCw className={`h-4 w-4 mr-1 ${mutation.isPending ? 'animate-spin' : ''}`} /> Restock Now
          </Button>
        </div>
      </CardContent>
    </Card>
