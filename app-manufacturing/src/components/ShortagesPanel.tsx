...
import { Package, Truck, ShoppingCart, DollarSign, Pencil, Ban, X } from 'lucide-react';
import { toast } from 'sonner';
import type { ManufacturingStatus } from '@/services/api';
import { fetchWorkOrders, updateWorkOrder, deleteWorkOrder } from '@/services/api';
import { fetchCatalog, formatIncomingEta, formatUtcTime, type SupplyQuote, type PurchaseOrder } from '@/services/supplyChainApi';
...
const ShortagesPanel: React.FC<Props> = ({ status, ordersByProduct }) => {
  ...
  const grouped = useMemo(() => {
    ...
    return Array.from(byProduct.values()).sort((a, b) => b.totalShortfall - a.totalShortfall);
  }, [shortages]);
  ...
  const toggleGroup = (ids: number[], on: boolean) => setSelected(prev => {
    const next = new Set(prev);
  ...
  const submitBulk = async () => {
    ...
    for (const id of ids) {
      try { await cancelMutation.mutateAsync(id); } catch { failures.push(id); }
    }
    const suffix = skipped > 0 ? `; skipped ${skipped} stale selection${skipped === 1 ? '' : 's'}` : '';
    if (failures.length === 0) toast.success(`Cancelled ${ids.length} work orders${suffix}`);
    else toast.error(`Cancelled ${ids.length - failures.length}/${ids.length}; failed: ${failures.join(', ')}${suffix}`);
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Work orders stalled waiting for purchased components</CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-muted-foreground text-sm text-center py-4">No active shortages — supply chain is healthy.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <>
      <Card>
        <CardHeader>
          <CardTitle className="font-doodle">Material Shortages</CardTitle>
          <CardDescription>
            Grouped by missing component. Use Reduce to scale a WO down, or Cancel to remove it entirely.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Accordion type="multiple" className="space-y-1">
            {grouped.map(g => {
              ...
              const best = quotes[0];
              const next = quotes[1];
              const estCost = best?.unitCost ? best.unitCost * remainingToOrder : null;
              const woIds = g.workOrders.map(w => w.workOrderId);
              const allSelected = woIds.length > 0 && woIds.every(id => selected.has(id));
              const someSelected = woIds.some(id => selected.has(id));

              return (
                <AccordionItem key={g.productId} value={String(g.productId)} className="border rounded-md px-3">
                  <AccordionTrigger className="py-3 hover:no-underline">
                    <div className="flex items-center gap-3 text-left flex-1 flex-wrap">
                      <Package className="h-4 w-4 text-muted-foreground shrink-0" />
                      <span className="font-medium text-sm">{g.productName}</span>
                      <Badge variant="destructive" className="text-xs font-mono">
                        short {g.totalShortfall.toLocaleString()}
                      </Badge>
                      <Badge variant="outline" className="text-xs">
                        {g.workOrders.length} WO{g.workOrders.length !== 1 ? 's' : ''} blocked
                      </Badge>
                      <span className="text-xs text-muted-foreground font-mono">
                        need {g.totalNeeded.toLocaleString()} · have {g.totalAvailable.toLocaleString()}
                      </span>
                      {totalOnOrder > 0 && (
                      ...
                      {estCost !== null && remainingToOrder > 0 && (
                        <Badge variant="secondary" className="text-xs gap-1" title={`${best!.unitCost.toFixed(2)}/ea from ${best!.vendorName}`}>
                      ...
                      {covered ? (
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/supply?product=${g.productId}&qty=${remainingToOrder}`);
                          }}
                        >
                  ...
                  <AccordionContent>
                    ...
                    {best && remainingToOrder > 0 && (
                      <div className="mb-3 p-2 bg-muted/40 rounded text-xs space-y-0.5">
                        <p className="font-medium text-muted-foreground">Cost preview</p>
                        <p>
                          <span className="font-mono">${best.unitCost.toFixed(2)}/ea</span>
                          {' × '}
                          <span className="font-mono">{remainingToOrder.toLocaleString()}</span>
                          {' = '}
                          <span className="font-mono font-bold">{fmtMoney(best.unitCost * remainingToOrder)}</span>
                          {' — '}
                          <span className="text-muted-foreground">{best.vendorName}</span>
                          {(best.stockAvailable ?? 0) < remainingToOrder && (
                            <span className="ml-2 text-destructive">(only {best.stockAvailable} in stock)</span>
                          )}
                        </p>
                        {next && (
                          <p className="text-muted-foreground">
                            Next: <span className="font-mono">${next.unitCost.toFixed(2)}/ea</span> — {next.vendorName}
                            {(next.stockAvailable ?? 0) > 0 && ` (${next.stockAvailable} in stock)`}
                          </p>
                        )}
                      </div>
                    )}
                    {orders.length > 0 && (
                      <div className="mb-3 p-2 bg-muted/40 rounded space-y-1">
                        <p className="text-xs font-medium text-muted-foreground mb-1">On order ({totalOnOrder.toLocaleString()} total)</p>
                        {orders.map((o, idx) => {
                          ...
                          return (
                            <div key={idx} className="flex items-center gap-1.5 text-xs flex-wrap">
                              ...
                              <span className="font-mono font-medium">{o.qty}×</span>
                              <span className="text-muted-foreground">from {o.vendorName}</span>
                              <Badge className="text-[10px] px-1.5 py-0">{o.status}</Badge>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {g.workOrders.sort((a, b) => b.shortfall - a.shortfall).map((s) => {
                          const wo = woById.get(s.workOrderId);
                          return (
                            <TableRow key={s.workOrderId} data-state={selected.has(s.workOrderId) ? 'selected' : undefined}>
                              <TableCell>
                                <Checkbox
                                  checked={selected.has(s.workOrderId)}
                                  onCheckedChange={() => toggle(s.workOrderId)}
      ...
      {selected.size > 0 && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 bg-background border-2 border-primary rounded-lg shadow-lg px-4 py-3 flex items-center gap-4">
          <div className="text-sm">
            <span className="font-bold">{selected.size}</span> WO{selected.size !== 1 ? 's' : ''} selected
            {bulkSavings.units > 0 && (
              <span className="text-muted-foreground">
                {' · '}frees ~{bulkSavings.units.toLocaleString()} units
                {bulkSavings.dollars > 0 && ` · saves ~${fmtMoney(bulkSavings.dollars)}`}
              </span>
            )}
          </div>
          <Button size="sm" variant="destructive" onClick={() => setBulkConfirm(true)} className="gap-1">
      ...
      <Dialog open={cancelOne != null} onOpenChange={(o) => !o && setCancelOne(null)}>
        <DialogContent className="doodle-dialog max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              This marks the work order as Rejected. Any stocked units remain in inventory. Consider reducing instead if you want to keep partial production.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={bulkConfirm} onOpenChange={setBulkConfirm}>
        <DialogContent className="doodle-dialog max-w-sm">
          <DialogHeader>
            <DialogTitle className="font-doodle">Cancel {selected.size} work orders?</DialogTitle>
            <DialogDescription>
              All selected WOs will be marked Rejected. Any already-stocked units stay in inventory.
              {bulkSavings.dollars > 0 && (
                <> Estimated material savings: <span className="font-mono font-bold">{fmtMoney(bulkSavings.dollars)}</span>.</>
              )}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setBulkConfirm(false)}>Keep them</Button>
            <Button variant="destructive" onClick={submitBulk} disabled={cancelMutation.isPending}>
