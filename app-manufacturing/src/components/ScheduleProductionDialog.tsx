...
const ScheduleProductionDialog = ({
  ...
  const effectiveSource: SuggestionSource = suggestionSource ?? (suggestedQty && suggestedQty > 0 ? 'open-demand' : 'reorder-point');
  ...
  const suggested = suggestedQty && suggestedQty > 0 ? suggestedQty : fallbackSuggested;

  const dueDefault = suggestedDueDate || new Date(Date.now() + product.DaysToManufacture * 86400000).toISOString().split('T')[0];

  const [form, setForm] = useState({ OrderQty: String(suggested), DueDate: dueDefault });
  ...
  const mutation = useMutation({
    mutationFn: () => beginManufacturingRun({
      productId: product.ProductID,
      orderQty: parseInt(form.OrderQty) || 1,
      dueDate: form.DueDate || undefined,
    }),
    onSuccess: (result) => {
      toast({ title: `✅ Manufacturing run started — WO #${result.rootWorkOrderId} (${result.totalWorkOrders} work orders)` });
  ...
  const earliestFeasible = new Date(today.getTime() + product.DaysToManufacture * 86400000);
  ...
  const dueTooSoon = !!dueDateObj && !dueInPast && dueDateObj < earliestFeasible;
  const orderQtyInvalid = !form.OrderQty || parseInt(form.OrderQty) < 1;
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="doodle-dialog max-w-md">
          <DialogHeader>
            <DialogTitle className="font-doodle text-lg font-bold text-doodle-text">Schedule Production: {product.Name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="doodle-card p-3 space-y-1">
              <div className="flex justify-between font-doodle text-sm">
                <span className="text-muted-foreground">Current Stock</span>
                <span className="font-bold text-doodle-accent">{currentQty}</span>
              </div>
              <div className="flex justify-between font-doodle text-sm">
                <span className="text-muted-foreground">Safety Stock Level</span>
              ...
              <div className="flex justify-between font-doodle text-sm">
                <span className="text-muted-foreground">Days to Manufacture</span>
                <span className="font-bold">{product.DaysToManufacture}</span>
              </div>
            </div>
            <div>
              <div className="flex items-center justify-between">
                <label className="font-doodle text-xs font-bold text-doodle-text">Order Quantity *</label>
                <TooltipProvider delayDuration={150}>
                  <Tooltip>
                    ...
                    <TooltipContent side="left" className="max-w-xs font-doodle text-xs">
              ...
              <input type="number" min="1" value={form.OrderQty} onChange={e => set('OrderQty', e.target.value)} className="doodle-input w-full text-sm mt-1" />
              <p className="font-doodle text-xs text-muted-foreground mt-1">
                Suggested: {suggested} {suggestionLabel ? `(${suggestionLabel})` : '(covers deficit + reorder point)'}
              </p>
            </div>
            <div>
              <label className="font-doodle text-xs font-bold text-doodle-text">Due Date</label>
              <input
                type="date"
                value={form.DueDate}
              ...
              />
              {dueDateError ? (
                <p className="font-doodle text-xs text-doodle-accent mt-1" role="alert">{dueDateError}</p>
              ) : (
                <p className="font-doodle text-xs text-muted-foreground mt-1">
                  Earliest feasible due date: {earliestFeasibleISO} (today + {product.DaysToManufacture}d lead time)
                </p>
              )}
            </div>
          </div>
