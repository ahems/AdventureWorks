...
> = ({ trigger, prefillProductId, prefillQty, onSuccess }) => {
  ...
  const { assembledIds: manufacturedIds, routedIds } =
    useManufacturedProductIds();
  const { data: inventory, isFetching: isFetchingInventory } = useQuery({
    queryKey: ["product-inventory-all"],
    queryFn: () => fetchProductInventory(),
  });
  const { data: allWorkOrders, isFetching: isFetchingWorkOrders } = useQuery({
  ...
  const stockByProduct = useMemo(() => {
    ...
    inventory?.forEach((inv) =>
      m.set(inv.ProductID, (m.get(inv.ProductID) || 0) + (inv.Quantity || 0)),
    );
    return m;
  }, [inventory]);
  ...
  const wipImpact = useMemo(() => {
    ...
    rows.sort((a, b) =>
      a.isTop === b.isTop ? a.name.localeCompare(b.name) : a.isTop ? -1 : 1,
    );
    return {
      rows,
  ...
  const handleOpen = useCallback(() => {
    ...
    if (prefillProductId && allProducts) {
      ...
      if (product) {
        ...
        setOrderQty(
          String(prefillQty || Math.max(product.ReorderPoint || 0, 1)),
        );
        // Set category/subcategory for breadcrumb
        if (product.ProductSubcategoryID) {
          const sub = subcategories?.find(
            (sc) => sc.ProductSubcategoryID === product.ProductSubcategoryID,
          );
          if (sub) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ["work-orders"] });
      toast({
        title: `✅ Production run started — ${res.totalWorkOrders} work orders created`,
      });
      onSuccess?.();
    } catch (e) {
      setError(String(e));
      setStep("review");
      toast({
        title: "❌ Failed to start production run",
  ...
  const selectedSubcategory = subcategories?.find(
    (sc) => sc.ProductSubcategoryID === subcategoryId,
  );
  ...
  return (
    <>
      ...
      <Dialog
        open={open}
        onOpenChange={(v) => {
          if (!v && step !== "submitting") {
            setOpen(false);
          }
        }}
      >
        <DialogContent
          className="doodle-dialog max-w-lg max-h-[85vh] overflow-y-auto"
        ...
        >
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                  ...
                  .map((sc) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          setSubcategoryId(sc.ProductSubcategoryID);
                          setStep("product");
                        }}
                        className="doodle-card-static p-3 text-left hover:border-doodle-blue transition-colors cursor-pointer"
                        disabled={count === 0}
                ...
                {filteredSubcategories.length === 0 && (
                  <p className="col-span-2 font-doodle text-sm text-muted-foreground text-center py-4">
          ...
          {step === "product" && (
            <div className="space-y-2">
              <p className="font-doodle text-sm text-muted-foreground">
                Select a finished product to manufacture:
              </p>
              <div className="space-y-1 max-h-[300px] overflow-y-auto">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="font-doodle text-xs text-muted-foreground">
                          {p.ProductNumber} · ${p.ListPrice.toFixed(2)}
                        </div>
                      </div>
                      <div className="shrink-0 text-right">
                        <div
                          className={`font-doodle text-sm font-bold ${out ? "text-doodle-accent" : low ? "text-doodle-accent" : "text-doodle-text"}`}
                        >
                          {stock}
                        </div>
                ...
                {filteredProducts.length === 0 && (
                  <p className="font-doodle text-sm text-muted-foreground text-center py-4">
                    No finished manufactured products in this subcategory
                  </p>
                )}
              </div>
            </div>
                ...
                <div className="space-y-4">
                  <div className="doodle-card-static p-3 bg-secondary/30 flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="font-doodle text-xs text-muted-foreground">
                        {selectedProduct.ProductNumber} · Days to manufacture:{" "}
                        {selectedProduct.DaysToManufacture}
                      </div>
                    </div>
                    <div className="shrink-0 text-right">
                      <div
                        className={`font-doodle text-base font-bold ${out || low ? "text-doodle-accent" : "text-doodle-text"}`}
                      >
                        {stock}
                      </div>
                      <div className="font-doodle text-[10px] text-muted-foreground uppercase tracking-wide">
                        {out ? "out of stock" : low ? "low stock" : "in stock"}{" "}
                        · reorder {selectedProduct.ReorderPoint}
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="font-doodle text-xs text-muted-foreground">
                      The API will explode the full BOM and create all required
                      work orders for sub-assemblies and the final product,
                      scheduling them in the correct dependency order.
                    </p>
                  </div>
                  ...
                  <div className="doodle-card-static p-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-doodle text-sm font-bold text-doodle-text flex items-center gap-2">
                        ...
                        {isWipRefreshing && (
                          <Loader2
                            ...
                            aria-label="Refreshing WIP data"
                          />
                        )}
                      </span>
                      <span className="font-doodle text-[11px] text-muted-foreground">
                        {isWipRefreshing
                          ? "Refreshing…"
                          : `${wipImpact.totalOpenWos} open WO${wipImpact.totalOpenWos === 1 ? "" : "s"} → ~${wipImpact.totalOpenWos + estimatedWorkOrders} after`}
                      </span>
                    </div>
                    {isWipRefreshing ? (
                      <div className="space-y-2">
                        <div className="grid grid-cols-3 gap-2 mb-3">
                          {Array.from({ length: 3 }).map((_, i) => (
                            <div
                              key={i}
                              className="rounded border border-doodle-text/15 p-2 space-y-2"
                            >
                      ...
                      <>
                        <div className="grid grid-cols-3 gap-2 text-center mb-3">
                          <div className="rounded border border-doodle-text/15 p-2">
                            <div className="font-doodle text-[10px] text-muted-foreground uppercase">
                              Open units now
                            </div>
                            <div className="font-doodle text-base font-bold text-doodle-text">
                              {wipImpact.totalOpenUnits.toLocaleString()}
                            </div>
                          </div>
                          <div className="rounded border border-doodle-text/15 p-2">
                            <div className="font-doodle text-[10px] text-muted-foreground uppercase">
                            ...
                            <div className="font-doodle text-base font-bold text-doodle-blue">
                              +{wipImpact.totalAddedUnits.toLocaleString()}
                            </div>
                          </div>
                          <div className="rounded border border-doodle-text/15 p-2">
                            <div className="font-doodle text-[10px] text-muted-foreground uppercase">
                            ...
                            <div className="font-doodle text-base font-bold text-doodle-text">
                              {(
                                wipImpact.totalOpenUnits +
                                wipImpact.totalAddedUnits
                              ).toLocaleString()}
                            </div>
                          </div>
                        </div>
                              ...
                              {wipImpact.rows.map((r) => (
                                ...
                                >
                                  <td className="py-1 pr-2 truncate max-w-[160px]">
                                    {r.name}
                                    {r.isTop && (
                                      <span className="ml-1 text-[10px] text-doodle-blue">
                                        (top)
                                      </span>
                                    )}
                                  </td>
                                  ...
                                  <td className="text-right py-1 pl-2 font-mono text-doodle-blue">
                                    +{r.addedUnits}
                                  </td>
                                  <td className="text-right py-1 pl-2 font-mono font-bold">
                                    {r.openUnits + r.addedUnits}
                                  </td>
                                </tr>
                              ))}
                            </tbody>
                        ...
                        <p className="font-doodle text-[11px] text-muted-foreground mt-2">
                          Open units = remaining (OrderQty − StockedQty) on work
                          orders not yet completed. New WOs start at 0 stocked
                          and add their full OrderQty to WIP.
                        </p>
                      </>
                    )}
                  </div>
          ...
          {step === "submitting" && (
            <div className="flex flex-col items-center justify-center py-8 gap-4">
              ...
              <p className="font-doodle text-xs text-muted-foreground">
                The API is exploding the BOM, creating work orders and
                scheduling operations. This may take a moment if the backend is
                waking up.
              </p>
            </div>
          )}
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="doodle-card-static p-3 text-center">
                  <div className="font-doodle text-lg font-bold text-doodle-blue">
                ...
                <div className="doodle-card-static p-3 text-center">
                  <div className="font-doodle text-lg font-bold text-doodle-green">
                    {result.leafWorkOrders}
                  </div>
                  <div className="font-doodle text-xs text-muted-foreground">
                    Started Processing
                  </div>
                ...
                <div className="doodle-card-static p-3 text-center">
                  <button
                    onClick={() => {
                      setOpen(false);
                      reset();
                      navigate(`/plan/work-orders/${result.rootWorkOrderId}`);
                    }}
                ...
                (() => {
                  ...
                  return (
                    <div className="space-y-2">
                      ...
                      <p className="font-doodle text-xs text-muted-foreground">
                        These components have insufficient stock. Production
                        will proceed and retry automatically until materials are
                        available.
                      </p>
                      <div className="space-y-1 max-h-[200px] overflow-y-auto">
                        {result.warnings.map((w, i) => {
                          const onOrder =
                            onOrderByProduct.get(w.productId) || 0;
                          ...
                          return (
                            <div
                              key={i}
                              className={`flex items-center justify-between px-3 py-2 rounded text-xs font-doodle ${effectiveShortfall === 0 ? "bg-primary/5 border border-primary/20" : "bg-amber-500/5 border border-amber-500/20"}`}
                            >
