...
const ReceiveProductInventory = () => {
  ...
  const { data: inventory, isLoading } = useQuery({
    queryKey: ["product-inventory", pid],
    queryFn: () => fetchProductInventory(pid),
  });
  ...
  const preferredVendorId = useMemo(() => {
    ...
    return (
      [...pool].sort((a, b) => a.unitCost - b.unitCost)[0]?.vendorId ?? null
    );
  }, [catalog]);
    ...
    <div className="container mx-auto px-4 py-8 space-y-6">
      ...
      <div className="doodle-card-static p-6">
        ...
        <p className="font-doodle text-sm text-muted-foreground">
          Total inventory:{" "}
          <span className="font-bold text-doodle-green">{totalQty}</span> units
        </p>
        {product && totalQty < product.SafetyStockLevel && (
          <div className="mt-3 p-3 rounded-lg border-2 border-doodle-accent/30 bg-doodle-accent/5 space-y-2">
            <p className="font-doodle text-sm text-doodle-accent font-bold">
              ⚠ Below safety stock — {totalQty} / {product.SafetyStockLevel} (
              {product.SafetyStockLevel - totalQty} deficit)
            </p>
              ...
              {!product.MakeFlag && (
                ...
                >
                  <Button
                    ...
                    className="gap-1.5 text-xs"
                  >
                    <ShoppingCart className="h-3.5 w-3.5" /> Purchase from
                    Vendor
                  </Button>
                </Link>
              )}
          ...
          {inventory?.map((inv, idx) => (
            <div key={idx} className="doodle-card p-4">
              <h3 className="font-doodle text-sm font-bold text-doodle-text">
                {locationMap.get(inv.LocationID) ||
