...
import { Package, ChevronDown, ChevronRight, Clock, PlayCircle, CheckCircle2, AlertTriangle } from 'lucide-react';
import { fetchWorkOrders, fetchManufacturedProducts, fetchProducts, fetchActiveBOM, fetchActiveOperations } from '@/services/api';
...
const WORow = ({ wo }: { wo: WorkOrder & { status: WOStatus; productName: string } }) => (
  <tr className="border-b border-doodle-text/10 hover:bg-secondary/30">
    <td className="py-2.5 px-4">
      <Link to={`/plan/work-orders/${wo.WorkOrderID}`} className="text-doodle-blue hover:underline font-bold">{wo.WorkOrderID}</Link>
    </td>
    <td className="py-2.5 px-4">
      <Link to={`/define/products/${wo.ProductID}`} className="hover:text-doodle-accent">{wo.productName}</Link>
    </td>
    <td className="text-right py-2.5 px-4">{wo.OrderQty}</td>
    <td className="text-right py-2.5 px-4">{wo.StockedQty}</td>
    <td className="text-right py-2.5 px-4">{wo.ScrappedQty > 0 ? <span className="text-doodle-accent">{wo.ScrappedQty}</span> : '0'}</td>
    <td className="text-center py-2.5 px-4">
      <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs border rounded ${statusColors[wo.status]}`}>
        {statusIcons[wo.status]} {wo.status}
      </span>
    </td>
    <td className="py-2.5 px-4">{new Date(wo.StartDate).toLocaleDateString()}</td>
...
const ProductGroupCard = ({ group, defaultExpanded }: { group: ProductGroup; defaultExpanded?: boolean }) => {
  ...
  return (
    <div className="doodle-card-static overflow-hidden">
      ...
      >
        ...
        <Package className={`w-4 h-4 shrink-0 ${group.isFinishedGood ? 'text-doodle-blue' : 'text-muted-foreground'}`} />
        <div className="flex-1 min-w-0">
          <div className="font-doodle text-sm font-bold text-doodle-text truncate">
            <Link to={`/define/products/${group.productId}`} className="hover:text-doodle-accent" onClick={e => e.stopPropagation()}>
              {group.productName}
            </Link>
            {group.isFinishedGood && <span className="ml-2 text-xs font-normal text-doodle-blue">(Finished Good)</span>}
            {group.componentCount > 0 && <span className="ml-2 text-xs font-normal text-muted-foreground">{group.componentCount} components</span>}
          </div>
        </div>
        <div className="flex items-center gap-3 shrink-0 font-doodle text-xs">
          <span className="text-muted-foreground">{group.orders.length} WO{group.orders.length !== 1 ? 's' : ''}</span>
          <span className="text-muted-foreground">Total: {group.totalQty}</span>
          <div className="flex gap-1">
            {Object.entries(group.statusCounts).map(([status, count]) => (
              <span key={status} className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 border rounded ${statusColors[status]}`}>
      ...
      {expanded && (
        <div className="overflow-x-auto border-t border-doodle-text/10">
          <table className="w-full font-doodle text-sm">
            <TableHead />
            <tbody>
              {group.orders.map(wo => <WORow key={wo.WorkOrderID} wo={wo} />)}
            </tbody>
          </table>
        </div>
      )}
...
const tabConfig = [
  { value: 'active', label: 'In Progress', icon: PlayCircle, statuses: ['In Progress'] as WOStatus[] },
  { value: 'upcoming', label: 'Upcoming', icon: Clock, statuses: ['Planned'] as WOStatus[] },
  { value: 'completed', label: 'Completed', icon: CheckCircle2, statuses: ['Completed', 'Scrapped'] as WOStatus[] },
];

const PlanWorkOrders = () => {
  const [searchParams] = useSearchParams();
  const initialTab = searchParams.get('status') === 'Completed' ? 'completed' : 'active';
  ...
  const finishedGoodIds = useMemo(() => {
    ...
    allProducts?.forEach(p => { if (p.FinishedGoodsFlag) s.add(p.ProductID); });
    return s;
  }, [allProducts]);
  ...
  const componentCounts = useMemo(() => {
    ...
    const countComponents = (productId: number, visited: Set<number>): number => {
      if (visited.has(productId)) return 0;
      visited.add(productId);
      const children = activeBom.filter(b => b.ProductAssemblyID === productId);
    ...
    const productIds = new Set(activeBom.map(b => b.ProductAssemblyID).filter((id): id is number => id != null));
  ...
  const enriched = useMemo(() =>
    (workOrders || []).map(wo => ({ ...wo, status: getStatus(wo), productName: productMap.get(wo.ProductID) || `#${wo.ProductID}` })),
  ...
  const statusCounts = useMemo(() => {
    const counts: Record<string, number> = { 'In Progress': 0, 'Planned': 0, 'Completed': 0, 'Scrapped': 0 };
    searchFiltered.forEach(wo => { counts[wo.status] = (counts[wo.status] || 0) + 1; });
    return counts;
  }, [searchFiltered]);

  const buildGroups = (statuses: WOStatus[], sortMode: 'due' | 'recent' | 'complexity') => {
    const filtered = searchFiltered.filter(wo => statuses.includes(wo.status));
    ...
    groups.forEach(g => {
      g.orders.sort((a, b) => new Date(a.DueDate).getTime() - new Date(b.DueDate).getTime());
    });
    ...
    if (sortMode === 'due') {
      ...
      groups.sort((a, b) => {
        const latestA = Math.max(...a.orders.map(o => new Date(o.EndDate || o.ModifiedDate).getTime()));
        const latestB = Math.max(...b.orders.map(o => new Date(o.EndDate || o.ModifiedDate).getTime()));
  ...
  const activeData = useMemo(() => buildGroups(['In Progress'], 'due'), [searchFiltered, finishedGoodIds, componentCounts]);
  const upcomingData = useMemo(() => buildGroups(['Planned'], 'due'), [searchFiltered, finishedGoodIds, componentCounts]);
  const completedData = useMemo(() => buildGroups(['Completed', 'Scrapped'], 'recent'), [searchFiltered, finishedGoodIds, componentCounts]);

  const renderTab = (data: { groups: ProductGroup[]; total: number }, defaultExpand: boolean) => (
    <div className="space-y-2">
      <div className="font-doodle text-xs text-muted-foreground">{data.groups.length} products · {data.total} work orders</div>
      {data.groups.length === 0 ? (
        <div className="doodle-card-static p-8 text-center font-doodle text-muted-foreground">No work orders in this category</div>
      ) : (
        data.groups.map(g => <ProductGroupCard key={g.productId} group={g} defaultExpanded={defaultExpand && data.groups.length <= 5} />)
      )}
    </div>
  );
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="font-doodle text-2xl font-bold text-doodle-text">3. Plan — Work Orders</h1>
          <p className="font-doodle text-sm text-muted-foreground">Track production progress — status synced with the shop floor</p>
        </div>
        <div className="flex gap-2">
          <CreateProductionOrderDialog />
          <Link to="/plan/schedule" className="doodle-button doodle-button-accent text-sm">Schedule View →</Link>
        </div>
      </div>
            ...
            {tabConfig.map(t => (
              <TabsTrigger key={t.value} value={t.value} className="flex items-center gap-1.5">
                ...
                <span className="ml-1 text-xs opacity-70">
                  ({t.statuses.reduce((sum, s) => sum + (statusCounts[s] || 0), 0)})
                </span>
              </TabsTrigger>
            ))}
          </TabsList>
