...
const PlanWorkOrderDetail = () => {
  ...
  const { data: routing } = useQuery({
    queryKey: ["wo-routing", woId],
    queryFn: () => fetchWorkOrderRouting(woId),
    enabled: !!wo,
    refetchInterval: 120_000,
  });
  const { data: productRouting } = useQuery({
    queryKey: ["product-routing", wo?.ProductID],
    queryFn: () => fetchRoutingByProduct(wo!.ProductID),
    enabled: !!wo,
  });
  const { data: product } = useQuery({
    queryKey: ["product", wo?.ProductID],
    queryFn: () => fetchProduct(wo!.ProductID),
    enabled: !!wo,
  });
  ...
  if (!wo)
    return (
      <div className="container mx-auto px-4 py-8 space-y-4">
        <Link
          to="/plan"
          className="inline-flex items-center gap-2 font-doodle text-doodle-blue hover:underline"
        >
          <ArrowLeft className="w-4 h-4" /> Back to Work Orders
        </Link>
        <div className="doodle-card-static p-6 text-center space-y-2">
          <AlertCircle className="w-8 h-8 text-muted-foreground mx-auto" />
          <h2 className="font-doodle text-xl font-bold">
            Work Order #{woId} not found
          </h2>
          <p className="text-sm text-muted-foreground">
            This work order may have been removed during a database reseed.
          </p>
        </div>
      </div>
    );
  ...
  const scrapReason = wo.ScrapReasonID
    ? scrapReasons?.find((s) => s.ScrapReasonID === wo.ScrapReasonID)
    : null;
  const sortedRouting = [...(routing || [])].sort(
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-6">
      ...
      <div className="doodle-card-static p-6">
        <div className="flex flex-col md:flex-row justify-between gap-4">
          <div>
            ...
            <Link
              ...
              className="font-doodle text-doodle-blue hover:underline"
            >
          ...
          <div className="flex gap-6 text-center">
            <div>
              ...
              <p className="font-doodle text-xl font-bold">{wo.OrderQty}</p>
            </div>
            <div>
              <p className="font-doodle text-xs text-muted-foreground">
                Stocked
        ...
        <div className="mt-4 space-y-2">
          <div className="flex justify-between font-doodle text-sm">
            <span>
              Progress: {wo.StockedQty} / {wo.OrderQty}
            </span>
          ...
          <div className="w-full h-4 bg-secondary border-2 border-doodle-text rounded-sm overflow-hidden">
            <div
              className="h-full bg-doodle-green transition-all"
          ...
          {status === "Scrapped" ? (
            ...
            statusSteps.map((step, i) => (
              <div key={step} className="flex items-center gap-2">
                {i <= currentStep ? (
                  <CheckCircle className="w-5 h-5 text-doodle-green" />
                ) : (
                  <Circle className="w-5 h-5 text-muted-foreground" />
                )}
                <span
                  className={`font-doodle text-sm ${i <= currentStep ? "font-bold text-doodle-text" : "text-muted-foreground"}`}
                >
                  {step}
                </span>
      ...
      <div className="doodle-card-static p-6">
        <h2 className="font-doodle text-lg font-bold text-doodle-text mb-4">
          Routing Schedule
        </h2>
        {sortedRouting.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full font-doodle text-sm">
              ...
              <tbody>
                {sortedRouting.map((r) => {
                  const overBudget =
                    r.ActualCost != null && r.ActualCost > r.PlannedCost;
                  const isActive = r.ActualStartDate && !r.ActualEndDate;
                  return (
                    <tr
                      key={`${r.WorkOrderID}-${r.OperationSequence}`}
                      className={`border-b border-doodle-text/10 ${isActive ? "bg-doodle-green/10" : ""}`}
                    >
                      <td className="py-2 px-3 font-bold">
                        {r.OperationSequence} {isActive && "🔄"}
                      </td>
                      <td className="py-2 px-3">
                        {locationMap.get(r.LocationID) || `#${r.LocationID}`}
                      </td>
                      <td className="py-2 px-3">
                        {new Date(r.ScheduledStartDate).toLocaleDateString()}
                      </td>
                      <td className="py-2 px-3">
                        {new Date(r.ScheduledEndDate).toLocaleDateString()}
                      </td>
                      <td className="py-2 px-3">
                        {r.ActualStartDate
                          ? new Date(r.ActualStartDate).toLocaleString()
                          : "—"}
                      </td>
                      <td className="py-2 px-3">
                        {r.ActualEndDate
                          ? new Date(r.ActualEndDate).toLocaleString()
                          : "—"}
                      </td>
                      <td className="text-right py-2 px-3">
                        ${r.PlannedCost.toFixed(2)}
                      </td>
                      <td
                        className={`text-right py-2 px-3 ${overBudget ? "text-doodle-accent font-bold" : ""}`}
                      >
                        {r.ActualCost != null
                      ...
                      <td className="text-right py-2 px-3">
                        {r.ActualResourceHrs != null
                          ? r.ActualResourceHrs.toFixed(2)
                          : "—"}
                      </td>
                    </tr>
                  );
          ...
          <p className="font-doodle text-sm text-muted-foreground">
            No routing steps yet — they'll be created when this work order is
      ...
      {(() => {
        const otherRouting = (productRouting || []).filter(
          (r) => r.WorkOrderID !== woId,
        );
        ...
        const sortedAllRouting = [...(productRouting || [])].sort(
          (a, b) => a.OperationSequence - b.OperationSequence,
        );
        const uniqueOps = [
          ...new Map(
            sortedAllRouting.map((r) => [
              `${r.LocationID}-${r.OperationSequence}`,
              r,
            ]),
          ).values(),
        ];
        ...
        return (
          <div className="doodle-card-static p-6">
            <h2 className="font-doodle text-lg font-bold text-doodle-text mb-2">
              Product Routing History
            </h2>
            <p className="font-doodle text-xs text-muted-foreground mb-4">
              All routing operations across{" "}
              {woGroups.size + (sortedRouting.length > 0 ? 1 : 0)} work order(s)
              for this product.
            </p>
            <div className="overflow-x-auto">
              <table className="w-full font-doodle text-sm">
                ...
                <tbody>
                  {uniqueOps.map((op) => {
                    const matching = (productRouting || []).filter(
                      (r) =>
                        r.LocationID === op.LocationID &&
                        r.OperationSequence === op.OperationSequence,
                    );
                    const avgPlanned =
                      matching.reduce((s, r) => s + r.PlannedCost, 0) /
                      matching.length;
                    const actuals = matching.filter(
                      (r) => r.ActualCost != null,
                    );
                    const avgActual =
                      actuals.length > 0
                        ? actuals.reduce((s, r) => s + (r.ActualCost || 0), 0) /
                          actuals.length
                        : null;
                    const hrsEntries = matching.filter(
                      (r) => r.ActualResourceHrs != null,
                    );
                    const avgHrs =
                      hrsEntries.length > 0
                        ? hrsEntries.reduce(
                            (s, r) => s + (r.ActualResourceHrs || 0),
                            0,
                          ) / hrsEntries.length
                        : null;
                    return (
                      <tr
                        key={`${op.LocationID}-${op.OperationSequence}`}
                      ...
                      >
                        ...
                        <td className="py-2 px-3">
                          {locationMap.get(op.LocationID) ||
                            `#${op.LocationID}`}
                        </td>
                        <td className="text-right py-2 px-3">
                          ${avgPlanned.toFixed(2)}
                        </td>
                        <td className="text-right py-2 px-3">
                          {avgActual != null ? `$${avgActual.toFixed(2)}` : "—"}
                        </td>
                        <td className="text-right py-2 px-3">
                          {avgHrs != null ? avgHrs.toFixed(2) : "—"}
                        </td>
                        <td className="text-right py-2 px-3">
                          {matching.length}
                        </td>
      ...
      <div className="flex gap-3">
        <Link
          to={`/execute/work-order/${woId}`}
          className="doodle-button doodle-button-accent text-sm"
        >
          Execution View →
        </Link>
        <Link
          to={`/engineer/bom/${wo.ProductID}`}
