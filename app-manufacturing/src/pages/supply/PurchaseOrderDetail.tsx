...
import {
  ArrowLeft, Truck, Package, Clock, ArrowRight, AlertTriangle,
  Star, Factory, ShoppingCart, MapPin, DollarSign, FileText, ExternalLink
} from 'lucide-react';
import { fetchOrder, fetchVendorDetail, type PurchaseOrder } from '@/services/supplyChainApi';
import { fetchProducts, fetchActiveBOM, fetchProductInventory } from '@/services/api';
...
const PurchaseOrderDetailPage: React.FC = () => {
  ...
  const { data: order, isLoading, error } = useQuery<PurchaseOrder>({
    queryKey: ['supply-order', orderId],
    queryFn: () => fetchOrder(orderId!),
    enabled: !!orderId,
    refetchInterval: (query) => {
      const status = query.state.data?.status;
      return status === 'pending' || status === 'approved' ? 5000 : false;
    },
  });
      ...
      <div className="container mx-auto p-6">
        <Card className="border-destructive">
          <CardContent className="p-6 text-center">
            ...
            <Link to="/supply" className="text-primary hover:underline text-sm mt-2 block">Back to Supply Chain</Link>
          </CardContent>
        </Card>
      </div>
    );
        ...
        <>
              ...
              <div className="flex flex-col md:flex-row items-start justify-between gap-4">
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge className={`${statusColors[order.status] || ''} text-sm px-3`} variant="secondary">
                      {order.status}
                    </Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Purchase Order for <strong>{order.productName}</strong> from <strong>{order.vendorName}</strong>
                  </p>
                </div>
              ...
              <CardContent>
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono">${order.unitCost.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="border-t pt-2 flex justify-between font-bold">
                    ...
                    <span className="font-mono text-lg">${order.totalCost.toFixed(2)}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span>{new Date(order.placedAtUtc).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Estimated Delivery</span>
                    <span>{new Date(order.estimatedDeliveryUtc).toLocaleString()}</span>
                  </div>
                  {order.actualDeliveryUtc && (
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Actual Delivery</span>
                      <span className="text-[hsl(var(--doodle-green))] font-medium">{new Date(order.actualDeliveryUtc).toLocaleString()}</span>
                    </div>
                  )}
                    ...
                    <div key={i} className="relative">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">{new Date(event.timestampUtc).toLocaleString()}</span>
                        </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="font-doodle text-base flex items-center gap-2">
                  <Package className="h-4 w-4 text-primary" /> Product: {order.productName}
                </CardTitle>
                <CardDescription>Component details and related pages</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Current Inventory</span>
                  <span className={`font-mono font-medium ${productStock === 0 ? 'text-destructive' : ''}`}>{productStock} units</span>
                </div>
                {product && (
                  <>
                    <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-mono">${product.StandardCost.toFixed(2)}</span>
                    </div>
                  </>
                )}
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="font-doodle text-base flex items-center gap-2">
                  <Truck className="h-4 w-4 text-primary" /> Vendor: {order.vendorName}
                  {v?.preferredVendorStatus && <Star className="h-4 w-4 text-yellow-500 fill-yellow-500" />}
                </CardTitle>
              ...
              <CardContent className="space-y-3">
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{(v.reliabilityPct * 100).toFixed(0)}%</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Lead Time</span>
                        <p className="font-medium">{v.defaultLeadTimeDays} days</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Ship Method</span>
                ...
                <div className="border-t pt-3 flex flex-wrap gap-2">
                  ...
                  <Link to={`/supply/vendors/${order.vendorId}?product=${order.productId}`}>
                    <Button variant="outline" size="sm" className="gap-1 text-xs">
          ...
          {dependentFinishedGoods.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="font-doodle text-base flex items-center gap-2">
                  <Factory className="h-4 w-4 text-primary" /> Finished Goods Using This Component ({dependentFinishedGoods.length})
                </CardTitle>
                <CardDescription>Products whose BOM includes {order.productName}</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                      ...
                      <TableRow key={fg.product.ProductID}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex gap-1 justify-end">
                            <Link to={`/define/products/${fg.product.ProductID}`}>
