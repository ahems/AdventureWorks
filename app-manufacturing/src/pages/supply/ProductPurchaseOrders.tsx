...
const ProductPurchaseOrders = () => {
  ...
  useEffect(() => {
    if (supplierId && pid) {
      navigate(`/supply/vendors/${supplierId}?product=${pid}&order=1`, {
        replace: true,
      });
    }
  ...
  const stats = useMemo(() => {
    const completed = orders.filter(
      (o) => o.status === "complete" && o.unitCost > 0,
    );
  ...
  const trend = stats
    ? stats.latest > stats.avg * 1.05
      ? "up"
      : stats.latest < stats.avg * 0.95
        ? "down"
        : "flat"
    : null;
    ...
    <div className="container mx-auto px-4 py-8 space-y-6">
      ...
      <div className="doodle-card-static p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <p className="font-doodle text-xs text-muted-foreground uppercase tracking-wide">
              Purchase History
            </p>
          ...
          <div className="flex gap-2 flex-wrap">
            <Link
              to={`/receive/inventory/${pid}`}
              className="doodle-button text-xs"
            >
              Inventory
            </Link>
            <Link
              to={`/supply?product=${pid}`}
              className="doodle-button text-xs"
            >
              Vendors & Quotes
            </Link>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <div className="doodle-card-static p-3 text-center">
            ...
            <div
              className={`flex items-center justify-center gap-1 ${trend === "up" ? "text-red-600" : trend === "down" ? "text-green-600" : "text-muted-foreground"}`}
            >
              {trend === "up" ? (
        ...
        <div className="doodle-card-static p-4">
          ...
          <div className="grid md:grid-cols-2 gap-2">
            {catalog.map((q, i) => (
              <div
                key={i}
                className="flex items-center justify-between gap-2 border border-border/50 rounded p-2 font-doodle text-xs"
              >
                <Link
                  to={`/supply/vendors/${q.vendorId}`}
                  className="text-doodle-blue hover:underline truncate"
                >
                  {q.vendorName}
                </Link>
                <div className="flex items-center gap-3 text-doodle-text">
                  ...
                  <Link
                    ...
                    title={
                      q.stockAvailable > 0
                        ? `Order from ${q.vendorName}`
                        : "Out of stock — view vendor"
                    }
                  >
                  ...
                  >
                    <td className="py-2">
                      <Link
                        ...
                        className="text-doodle-blue hover:underline font-mono"
                      >
                        {o.orderId}
                      </Link>
                    </td>
                    ...
                    <td className="py-2">
                      <Link
                        ...
                        className="text-doodle-text hover:underline"
                      >
                        {o.vendorName}
                      </Link>
                    </td>
