...
function creditBadge(rating: number) {
  ...
  return (
    <Badge className={`${colors[rating]} text-xs`}>Rating {rating}/5</Badge>
  );
}
...
const SupplyChain: React.FC = () => {
  ...
  const completedOrders =
    orderHistory?.filter((o) => ["complete", "rejected"].includes(o.status)) ??
    [];
        ...
        <Card className="border-destructive">
          <CardContent className="p-6 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Could not connect to the supply chain API. Please try again.
            </p>
            <Button
              variant="destructive"
              onClick={() =>
          ...
          <CardContent className="p-3 flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span>
                ...
                <strong>{productName || `Product #${productFilter}`}</strong>
              </span>
              <Badge variant="secondary" className="text-xs">
                {vendorIdsForProduct.size} vendor
                {vendorIdsForProduct.size !== 1 ? "s" : ""}
              </Badge>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Card>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold font-doodle">
                    {
                      vendors.filter((v) => v.vendor.preferredVendorStatus)
                        .length
                    }
                  </p>
            ...
            <Card>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold font-doodle">
                    {vendors.reduce((s, v) => s + v.deliveredToday, 0)}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Delivered Today
                  </p>
      ...
      <Tabs value={tab} onValueChange={setTab} className="space-y-4">
        <TabsList className="grid w-full grid-cols-4">
          <TabsTrigger value="vendors">
            <Package className="h-4 w-4 mr-1 hidden sm:inline" /> Vendors
          </TabsTrigger>
          <TabsTrigger value="risk">
            <ShieldAlert className="h-4 w-4 mr-1 hidden sm:inline" /> Risk
            Analysis
          </TabsTrigger>
          <TabsTrigger value="active">
            <ShoppingCart className="h-4 w-4 mr-1 hidden sm:inline" /> Active
            Orders{activeOrders.length ? ` (${activeOrders.length})` : ""}
          </TabsTrigger>
          <TabsTrigger value="history">
            <Clock className="h-4 w-4 mr-1 hidden sm:inline" /> History
          </TabsTrigger>
        </TabsList>
              ...
              {vendors
                ...
                .sort((a, b) => {
                  ...
                  if (productFilter && vendorIdsForProduct.size > 0) {
                    const aMatch = vendorIdsForProduct.has(a.vendor.vendorId)
                      ? 0
                      : 1;
                    const bMatch = vendorIdsForProduct.has(b.vendor.vendorId)
                      ? 0
                      : 1;
                ...
                .map((vs) => (
                  <VendorCard
                    ...
                    highlighted={
                      productFilter
                        ? vendorIdsForProduct.has(vs.vendor.vendorId)
                        : false
                    }
        ...
        <TabsContent value="risk">
          <Tabs defaultValue="vendor-blockers" className="space-y-4">
            <TabsList>
              <TabsTrigger value="vendor-blockers">
                <PackageX className="h-4 w-4 mr-1" /> Vendor Blockers
              </TabsTrigger>
              <TabsTrigger value="finished-goods">
                <Factory className="h-4 w-4 mr-1" /> Finished Good Risk
              </TabsTrigger>
              <TabsTrigger value="components">
                <Layers className="h-4 w-4 mr-1" /> Component Sourcing
              </TabsTrigger>
            </TabsList>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <Card>
                <CardContent className="p-3 flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-lg font-bold font-doodle">
                      {activeOrders.length}
                    </p>
              ...
              <Card>
                <CardContent className="p-3 flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-lg font-bold font-doodle">
                      {activeOrders.reduce((s, o) => s + o.qty, 0)}
                    </p>
                    <p className="text-[11px] text-muted-foreground">
                      Total Units
                    </p>
                  ...
                  <TableBody>
                    {[...activeOrders]
                      .sort((a, b) => {
                        ...
                        return (
                          new Date(a.estimatedDeliveryUtc).getTime() -
                          new Date(b.estimatedDeliveryUtc).getTime()
                        );
                      })
                      .map((o) => (
                        <TableRow key={o.orderId}>
                          ...
                          <TableCell>
                            <Link
                              to={`/receive/inventory/${o.productId}`}
                              className="text-sm hover:underline text-primary"
                            >
                              {o.productName}
                            </Link>
                          ...
                          <TableCell>
                            {o.status === "pending" && (
                              <Button
                                ...
                                onClick={(e) => {
                                  e.preventDefault();
                                  cancelMutation.mutate(o.orderId);
                                }}
                  ...
                  <TableBody>
                    {completedOrders.slice(0, 50).map((o) => (
                      <TableRow key={o.orderId}>
                        ...
                        <TableCell className="text-xs text-muted-foreground">
                          {o.actualDeliveryUtc
                            ? new Date(o.actualDeliveryUtc).toLocaleString()
                            : "—"}
                        </TableCell>
                      </TableRow>
                    ))}
...
function VendorCard({
  ...
  const v = vs.vendor;
  const stockPct =
    vs.totalComponents > 0
      ? Math.round((vs.inStockComponents / vs.totalComponents) * 100)
      : 0;
  const linkTo = productFilter
    ? `/supply/vendors/${v.vendorId}?product=${productFilter}`
    : `/supply/vendors/${v.vendorId}`;
  return (
    <Link to={linkTo}>
      <Card
        className={`hover:ring-2 hover:ring-primary/30 transition-shadow cursor-pointer ${highlighted ? "ring-2 ring-primary border-primary" : ""} ${highlighted === false && productFilter ? "opacity-40" : ""}`}
      >
        <CardContent className="p-4">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="font-doodle font-bold text-base truncate">
                  {v.name}
                </h3>
            ...
            <div className="text-right shrink-0 space-y-1">
              <div className="text-sm">
                <span className="font-mono font-bold">
                  {vs.inStockComponents}
                </span>
                <span className="text-muted-foreground">
                  /{vs.totalComponents} in stock
                </span>
              </div>
