...
export async function fetchCatalog(productId?: number): Promise<SupplyQuote[]> {
  const url = productId ? `${BASE}/supply/catalog/${productId}` : `${BASE}/supply/catalog`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Catalog error: ${res.status}`);
  return res.json();
}

export async function fetchQuote(vendorId: string, productId: number, qty: number): Promise<SupplyQuote> {
  const res = await fetch(`${BASE}/supply/quote?vendorId=${vendorId}&productId=${productId}&qty=${qty}`);
  if (!res.ok) throw new Error(`Quote error: ${res.status}`);
  return res.json();
}

export async function placeOrder(vendorId: string, productId: number, qty: number): Promise<PurchaseOrder> {
...
export async function fetchOrder(orderId: string): Promise<PurchaseOrder> {
  ...
  const unitCostSum = lines.reduce((s, l) => s + (l.unitCost || 0) * (l.qty || 0), 0);
  return {
    ...first,
    qty,
    ...
    productName: lines.length > 1 ? `${lines.length} line items` : first.productName,
  };
}
...
export async function cancelOrder(orderId: string, reason?: string): Promise<{ message?: string; error?: string }> {
  const res = await fetch(`${BASE}/supply/order/${orderId}`, {
    method: 'DELETE',
    headers: { 'Content-Type': 'application/json' },
...
export async function restockVendor(vendorId: string): Promise<{ message: string }> {
  const res = await fetch(`${BASE}/supply/restock/${vendorId}`, { method: 'POST' });
