...
import type { SalesOrderHeader, SalesOrderDetail, ODataResponse } from '@/types/production';

const API_BASE = ODATA_BASE;

async function fetchAll<T>(entity: string, params?: Record<string, string>, maxPages = 50): Promise<T[]> {
  const all: T[] = [];
...
export async function fetchSalesOrderDetailsForOrders(orderIds: number[]): Promise<SalesOrderDetail[]> {
  ...
  for (let i = 0; i < orderIds.length; i += CHUNK) {
    ...
    const rows = await fetchAll<SalesOrderDetail>('SalesOrderDetail', { $filter: filter });
    out.push(...rows);
  }
  return out;
}
...
export interface OpenDemandRow {
  ...
  eShopQty: number;
  b2bQty: number;
  orderRefs: { salesOrderId: number; salesOrderNumber: string; qty: number; dueDate: string; online: boolean; status: number; customerId: number }[];
}
...
export async function loadOpenDemand(): Promise<{ headers: SalesOrderHeader[]; rows: OpenDemandRow[] }> {
  const headers = await fetchOpenSalesOrders();
  if (headers.length === 0) return { headers, rows: [] };
  const details = await fetchSalesOrderDetailsForOrders(headers.map((h) => h.SalesOrderID));
  ...
  for (const d of details) {
    ...
    if (new Date(h.DueDate) < new Date(row.earliestDueDate)) row.earliestDueDate = h.DueDate;
    row.orderRefs.push({
  ...
  return { headers, rows: [...grouped.values()].sort((a, b) => b.openQty - a.openQty) };
}

export async function loadHistoricalDemand(anchorDate: Date): Promise<HistoricalDemandRow[]> {
  ...
  for (const d of details) {
    ...
    if (!row) {
      row = { productId: d.ProductID, qty30d: 0, qty90d: 0, qty365d: 0, eShopQty: 0, b2bQty: 0, lastOrderDate: h.OrderDate, revenue90d: 0 };
    ...
    if (new Date(h.OrderDate) > new Date(row.lastOrderDate)) row.lastOrderDate = h.OrderDate;
  }
  return [...map.values()];
}
