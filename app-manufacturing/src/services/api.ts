...
function buildODataUrl(base: string, params?: Record<string, string>): string {
...
async function fetchOData<T>(
  ...
  const url = buildODataUrl(`${API_BASE}/${entity}`, params);
...
async function fetchAllOData<T>(
  entity: string,
  params?: Record<string, string>,
  maxPages = 50,
): Promise<T[]> {
  const all: T[] = [];
  let url: string | null = buildODataUrl(`${API_BASE}/${entity}`, params);
  let page = 0;
...
export const fetchProduct = (id: number) =>
  fetchOData<Product>("Product", { $filter: `ProductID eq ${id}` }).then(
    (r) => r[0] || null,
  );

// Product Models
export const fetchProductModels = () =>
  fetchAllOData<ProductModel>("ProductModel");
export const fetchProductModel = (id: number) =>
  fetchOData<ProductModel>("ProductModel", {
    $filter: `ProductModelID eq ${id}`,
  }).then((r) => r[0] || null);

// Categories (filter to English only — these tables have CultureID)
export const fetchProductCategories = () =>
  fetchAllOData<ProductCategory>("ProductCategory", {
    $filter: "CultureID eq 'en'",
  });
export const fetchProductSubcategories = () =>
  fetchAllOData<ProductSubcategory>("ProductSubcategory", {
    $filter: "CultureID eq 'en'",
  });

// Descriptions
export const fetchProductDescriptions = () =>
  fetchAllOData<ProductDescription>("ProductDescription");
export const fetchProductModelDescCultures = (modelId?: number) =>
  fetchAllOData<ProductModelProductDescriptionCulture>(
    "ProductModelProductDescriptionCulture",
    modelId
      ? { $filter: `ProductModelID eq ${modelId} and CultureID eq 'en'` }
      : { $filter: "CultureID eq 'en'" },
  );

// Unit Measures
export const fetchUnitMeasures = () =>
  fetchAllOData<UnitMeasure>("UnitMeasure");
...
export const fetchWorkOrders = () =>
  fetchOData<WorkOrder>("WorkOrder", {
    $orderby: "ModifiedDate desc",
    $first: "500",
  });

export const fetchWorkOrder = (id: number) =>
  fetchOData<WorkOrder>("WorkOrder", { $filter: `WorkOrderID eq ${id}` }).then(
    (r) => r[0] || null,
  );

export const fetchWorkOrdersByProduct = (productId: number) =>
  fetchAllOData<WorkOrder>(
    "WorkOrder",
...
export const fetchLocation = (id: number) =>
  fetchOData<Location>("Location", { $filter: `LocationID eq ${id}` }).then(
    (r) => r[0] || null,
  );

// Scrap Reasons
export const fetchScrapReasons = () =>
  fetchAllOData<ScrapReason>("ScrapReason");
...
export const createProduct = (body: Record<string, unknown>) =>
  postEntity<Product>("Product", body);
export const deleteProduct = (id: number) =>
  deleteEntity("Product", `ProductID/${id}`);

// BOM
export const createBOM = (body: Record<string, unknown>) =>
  postEntity<BillOfMaterials>("BillOfMaterials", body);
export const deleteBOM = (id: number) =>
  deleteEntity("BillOfMaterials", `BillOfMaterialsID/${id}`);

// Work Orders
export const createWorkOrder = (body: Record<string, unknown>) =>
  postEntity<WorkOrder>("WorkOrder", body);
export const deleteWorkOrder = (id: number) =>
  deleteEntity("WorkOrder", `WorkOrderID/${id}`, { ignoreNotFound: true });

// Work Order Routing
export const createWorkOrderRouting = (body: Record<string, unknown>) =>
  postEntity<WorkOrderRouting>("WorkOrderRouting", body);
export const deleteWorkOrderRouting = (
  woId: number,
  productId: number,
  opSeq: number,
) =>
  deleteEntity(
    "WorkOrderRouting",
    `WorkOrderID/${woId}/ProductID/${productId}/OperationSequence/${opSeq}`,
  );

// Product Models CRUD
export const createProductModel = (body: Record<string, unknown>) =>
  postEntity<ProductModel>("ProductModel", body);
export const deleteProductModel = (id: number) =>
  deleteEntity("ProductModel", `ProductModelID/${id}`);
...
export async function stopManufacturing(): Promise<void> {
  const res = await fetch(`${MANUFACTURING_BASE}/manufacturing/stop`, {
    method: "POST",
  });
...
export async function beginManufacturingRun(
  body: ManufacturingBeginRequest,
): Promise<ManufacturingBeginResponse> {
...
export async function updateScrapConfig(
  locationId: number,
  body: { failureRatePct: number; scrapReasonIds: number[]; note?: string },
): Promise<ScrapConfig> {
  const res = await fetch(
    `${MANUFACTURING_BASE}/manufacturing/scrap-config/${locationId}`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
...
export async function fetchLocationConfig(): Promise<LocationConfig[]> {
  const res = await fetch(
    `${MANUFACTURING_BASE}/manufacturing/location-config`,
  );
...
export async function updateLocationConfig(
  ...
  const res = await fetch(
    `${MANUFACTURING_BASE}/manufacturing/location-config/${locationId}`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
...
export async function fetchScrapEvents(
  vendorId?: number,
): Promise<ScrapEventFull[]> {
  const url = vendorId
